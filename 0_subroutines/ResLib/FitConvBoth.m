function [pa,dpa,chisqN,sim,CN,PQ,nit,kvg,details]=...
   FitConvBoth(H,K,L,W,EXP,Iobs,dIobs,sqw,pref,sma,smapref,pa,ia,METHOD, ACCURACY, SMACCURACY,nitmax,tol,dtol)
%==================================================================================
%function [pa,dpa,chisqN,sim,CN,PQ,nit,kvg,details]=...
%   FitConvBoth(H,K,L,W,EXP,Iobs,dIobs,sqw,pref,sma,smapref,pa,ia,METHOD, ACCURACY, SMACCURACY,nitmax,tol,dtol)
%
%  ResLib v.3.4
%==================================================================================
% Fit a parametrized model cross section that is a sum of a sandard and an SMA-stlyle 
% user-supplied functions, convoluted with the spectrometer resolution function to 
% experimental 3-axis data. Levenberg-Marquardt least squares algorithm is used. 
% See NR 15.5. The current version does not support analytic derivatives.
%
%
...
if (nargin < 14 | isempty(METHOD)), METHOD='fix2'; end %warning('Using default "fix2" integration method'); end;
if (nargin < 15 | isempty(ACCURACY)), ACCURACY=[7 0]; end %warning('Using default density of point sampling: ACCURACY=[7 0]'); end; 
if (nargin < 16 | isempty(SMACCURACY)), SMACCURACY=[7 0]; end %warning('Using default density of point sampling: SMA ACCURACY=[7 0]'); end; 
if (nargin < 17 | isempty(nitmax)), nitmax=20; end %warning('Using default limit on number of iterations: nitmax=20'); end; 
if (nargin < 18 | isempty(tol)), tol=0.001; end %warning('Using default tolerance: tol=0.001'); end; 
if (nargin < 19 | isempty(dtol)), dtol=1e-5; end %warning('Using default tolerance: dtol=1e-5'); end;
...
dispia=(ia>0);
fprintf('Fitting ''%s'' and ''%s'' to %d data using %d free and %d fixed parameters.\n',sqw,sma,length(Iobs),sum(dispia), length(pa)-sum(dispia)); 
fprintf('\n');

%----------------------------------------------------------------------------------------------------------------
[chisq,alpha,beta]=marqit(H,K,L,W,EXP,Iobs,dIobs,sqw,pref,sma,smapref,pa,ivar,METHOD,ACCURACY,SMACCURACY,tol,dtol);
...
while (abs(chisq_old-chisq)> tol*chisq)
   if (nit >=nitmax),break,end 
    ...
    [chisq_trial]=marqit(H,K,L,W,EXP,Iobs,dIobs,sqw,pref,sma,smapref,pt,ivar,METHOD,ACCURACY,SMACCURACY,tol,dtol);% get trial chi_squared
   if (chisq_trial > chisq) % chisq increases ?
      ...
      if(lamda > 1e13),chisq_old=chisq;end %punt if lamda is stuck beyond reason
        ...
        [chisq,alpha,beta]=marqit(H,K,L,W,EXP,Iobs,dIobs,sqw,pref,sma,smapref,pa,ivar,METHOD,ACCURACY,SMACCURACY,tol,dtol); %recalculate alpha,beta,chi-squared
...
dpa(ivar)=sqrt(chisqN*diag(C)); % error in a(j) is sqrt(Cjj)
CN=C./sqrt(abs(diag(C)*diag(C)')); % normalized correlation matrix C(ij)/sqrt[C(ii)*C(jj)]
if (nargout >=5),PQ=1-gammainc(chisq/2,DF/2); end
%----------------------------------------------------------------------------------------------------------------
if (nargout>=8)
    sim = ConvResSMA(sma,smapref,H,K,L,W,EXP,METHOD,SMACCURACY,pa);
    sim = sim+ConvRes(sqw,pref,H,K,L,W,EXP,METHOD,ACCURACY,pa);
...
function dfp=dfdp(H,K,L,W,EXP,Iobs,dIobs,sqw,pref,sma,smapref,pa,ivar,METHOD,ACCURACY,SMACCURACY,dtol)
global GLOBAL_FUNCOUNT____
np=length(H);
...
for n=1:length(ivar) %loop through varying parameters
   ...
   pa1=pa+h;
   pa2=pa-h;
   f1=ConvResSMA(sma,smapref,H,K,L,W,EXP,METHOD,SMACCURACY,pa1);GLOBAL_FUNCOUNT____=GLOBAL_FUNCOUNT____+1;
   f2=ConvResSMA(sma,smapref,H,K,L,W,EXP,METHOD,SMACCURACY,pa2);GLOBAL_FUNCOUNT____=GLOBAL_FUNCOUNT____+1;
   f1=f1+ConvRes(sqw,pref,H,K,L,W,EXP,METHOD,ACCURACY,pa1);GLOBAL_FUNCOUNT____=GLOBAL_FUNCOUNT____+1;
   f2=f2+ConvRes(sqw,pref,H,K,L,W,EXP,METHOD,ACCURACY,pa2);GLOBAL_FUNCOUNT____=GLOBAL_FUNCOUNT____+1;
   dfp(:,n)=(f1(:)-f2(:))/(2*h(ivar(n)));
end
%=================================================================================================================

function [chisq,alpha,beta] = marqit(H,K,L,W,EXP,Iobs,dIobs,sqw,pref,sma,smapref,pa,ivar,METHOD,ACCURACY,SMACCURACY,tol,dtol)
global GLOBAL_FUNCOUNT____
   f=ConvResSMA(sma,smapref,H,K,L,W,EXP,METHOD,SMACCURACY,pa); GLOBAL_FUNCOUNT____=GLOBAL_FUNCOUNT____+1;
   f=f+ConvRes(sqw,pref,H,K,L,W,EXP,METHOD,ACCURACY,pa); GLOBAL_FUNCOUNT____=GLOBAL_FUNCOUNT____+1;
   wdiff=(Iobs-f)./dIobs; %weighted difference
   chisq=sum(wdiff.^2); % chi_squared
   if  nargout==1, return; end;%only chisq requested
   dfp=dfdp(H,K,L,W,EXP,Iobs,dIobs,sqw,pref,sma,smapref,pa,ivar,METHOD,ACCURACY,SMACCURACY,dtol);   
